        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 mb-6 md:mb-12 w-full">
            ...
            <div class="bg-[#141418]/50 border border-white/5 rounded-2xl md:rounded-[1.5rem] p-4 md:p-7 flex items-center gap-4 md:gap-5 stats-cyan transition-transform hover:-translate-y-1 w-full min-w-0">
                ...
                <div class="min-w-0">
                    ...
                    <span class="text-xl md:text-2xl lg:text-[1.6rem] font-black text-white block mt-0.5 truncate">{{ $totalHoursPlayed ?? '0' }} JAM</span>
                </div>
            </div>
                    ...
                    @forelse($recentActivities as $activity)
                        ...
                        <div class="flex items-center gap-3 md:gap-4 py-3 md:py-4 border-b border-white/5 last:border-0 w-full min-w-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <span class="text-white font-bold block text-sm md:text-base truncate">Booking {{ strtoupper($tableName) }}</span>
                                <span class="text-white/20 text-[0.65rem] md:text-sm truncate block">{{ \Carbon\Carbon::parse($activity->booking_date)->format('d M Y') }}, {{ \Carbon\Carbon::parse($activity->start_time)->format('H:i') }} WIB</span>
                            </div>
                            <span class="text-[0.6rem] md:text-xs font-black uppercase text-[#00f2ff] bg-[#00f2ff]/10 px-2 py-1 md:px-2.5 rounded-md border border-[#00f2ff]/20 shrink-0">{{ $statusText }}</span>
                        </div>
                    @empty
